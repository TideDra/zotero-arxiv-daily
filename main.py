...
def get_arxiv_paper(query: str, debug: bool = False) -> list[ArxivPaper]:
    ...
    #
    date_query = f"{query} AND submittedDate:[{(datetime.datetime.now() - datetime.timedelta(days=3)).strftime('%Y-%m-%d')} TO {datetime.datetime.now().strftime('%Y-%m-%d')}]"
    feed = feedparser.parse(f"https://export.arxiv.org/api/query?search_query={date_query}")
