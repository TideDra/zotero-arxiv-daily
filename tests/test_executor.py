...
def test_run_handles_unexpected_send_email_exception(config, monkeypatch):
    import smtplib
    ...
    monkeypatch.setattr(registered_retrievers["arxiv"], "retrieve_papers", lambda self: [])
    def _raise_smtp_disconnect(*_args, **_kwargs):
        raise smtplib.SMTPServerDisconnected("Connection unexpectedly closed")

    monkeypatch.setattr("zotero_arxiv_daily.executor.send_email", _raise_smtp_disconnect)
    monkeypatch.setattr("zotero_arxiv_daily.retriever.base.sleep", lambda _: None)
